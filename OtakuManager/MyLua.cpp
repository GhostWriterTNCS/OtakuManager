...
static int lua_print(lua_State* L) {
	switch (lua_type(L, lua_gettop(L))) {
		case LUA_TNUMBER:
			std::cout << "Lua number: " << lua_tonumber(L, lua_gettop(L)) << std::endl;
			break;
		case LUA_TTABLE:
			std::cout << "Lua table of " << lua_rawlen(L, 1) << " objects:" << std::endl;
			lua_pushnil(L);
			while (lua_next(L, -2)) {
				lua_pushvalue(L, -2);
			...
			break;
		case LUA_TSTRING:
			std::cout << "Lua string: " << lua_tostring(L, lua_gettop(L)) << std::endl;
			break;
		case LUA_TBOOLEAN:
			std::cout << "Lua boolean: " << lua_toboolean(L, lua_gettop(L)) << std::endl;
			break;
		default:
...
bool getEpisodesByUrl(QString website, QString url, QList<Episode>* episodes) {
	...
	// the function name
	lua_getglobal(L, "getEpisodesByUrl");
...
QString goToEpisode(QString website, Episode* episode, QString type) {
	...
	int pcall_return = lua_pcall(L, 2, 1, 0);
	if (pcall_return > 0) {
		std::cout << "lua_pcall for method 'goToEpisode' in module '" << website.toStdString()
				  << "' returned error " << pcall_return << ". Using current episode url."
				  << std::endl;
